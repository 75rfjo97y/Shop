...
def console_generate_template(text_value, template=None):
    ...
    # pattern as the requested reference template. This is intentionally
    # limited to console_generate_template().
    console_line_emoji = [
        (r"(?im)^.*WITHDRAWAL\s+SUCCESSFUL.*$", console_pe_aliases["withdraw_success"]),
        (r"(?im)^.*Amount\s+Paid:.*$", console_pe_aliases["withdraw_amount"]),
        (r"(?im)^.*Withdrawal\s+Fee:.*$", None),
...
async def console_send_test(m):
    ...
    try:
        ...
        return await ans(m, "Test message forwarded to target.", reply_markup=console_settings_kb())
    except Exception as e:
