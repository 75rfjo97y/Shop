...
def ibtn(text, **kwargs):
    ...
    try:
        ...
        kwargs.pop("style", None)
        return _InlineKeyboardButton(text=text, **kwargs)
TOKEN = "8955454814:AAGbgdTHmkCZxHJ_gZNvBI2xEwnTwjAE4Ew"
ADMIN_ID = 6995426618
...
def _pending_submissions(category_name=""):
    if category_name:
        return c.execute(
...
async def process_report(bot, report_id, report_raw, category_name="", category_id=None):
    """
    Automatically process every pending submission in the selected category.
    ...
    rows=_pending_submissions(category_name)
    user_files={}
    for s in rows:
...
def console_load_data():
    if not CONSOLE_DATA_FILE.exists():
        ...
        return dict(CONSOLE_DEFAULT_DATA)
    try:
...
def console_generate_template(text_value, template=None):
    ...
    def bal(m):
        ...
        return str(accepted * int(m.group(1)))

    result = re.sub(r"\{balance:\s*accepted\s*\*\s*(\d+)\}", bal, result, flags=re.I)
...
async def console_send_test(m):
    ...
    t = d["templates"][idx]
    if not t["channel_id"]:
        return await ans(m, "এই Template-এর Channel আগে সেট করুন.", reply_markup=console_settings_kb())
    msg = console_generate_template(t["text"], t)
    try:
        await m.bot.send_message(chat_id=t["channel_id"], text=msg)
        return await ans(m, "Test message sent.", reply_markup=console_settings_kb())
    except Exception as e:
        return await ans(m, f"Send failed: {e}", reply_markup=console_settings_kb())
...
async def console_action(m):
    ...
    if ttext in (
        ...
        if ttext == "View Template":
            return await ans(m, tt["text"], reply_markup=console_settings_kb())
    ...
    if action == "amount_rules":
        ...
        console_save_data(d)
        state[ADMIN_ID].pop("console_action", None)
        return await ans(m, f"Template {idx+1} amount rules saved.\n\n{console_format_amount_rules(tt)}", reply_markup=console_settings_kb())
    ...
    if action == "channel":
        ...
        console_save_data(d)
        state[ADMIN_ID].pop("console_action", None)
        return await ans(m, "Channel saved.", reply_markup=console_settings_kb())
    ...
    if action == "time":
        ...
        console_save_data(d)
        state[ADMIN_ID].pop("console_action", None)
        return await ans(m, "Template time saved.", reply_markup=console_settings_kb())
    ...
    if action == "limit":
        ...
        console_save_data(d)
        state[ADMIN_ID].pop("console_action", None)
        return await ans(m, "Template message limit saved.", reply_markup=console_settings_kb())
                ...
                for _ in range(count):
                    ...
                    try:
                        await bot.send_message(
                            chat_id=current_t["channel_id"],
                            text=console_generate_template(current_t["text"], current_t),
                        )
                    except Exception as e:
...
async def report_admin_action(m):
    ...
    if t=="Upload Report File":
        cleanup_expired_report()
        # Report upload is allowed even when the selected Work is disabled.
...
async def report_doc(m):
    ...
    c.execute(
        "INSERT INTO reports(file_name,file_path,uploaded_at,expires_at,green_count,category_name) "
        "VALUES(?,?,?,?,?,?)",
        (fn,str(report_path),uploaded,expires,0,selected_category)
    )
    rid=c.lastrowid
    db.commit()
    state.pop(ADMIN_ID,None)
    ...
    results,total_reward,processed_users,green_count=await process_report(
        m.bot,rid,raw,selected_category
    )
