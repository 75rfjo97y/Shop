...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8675144844:AAFGgQKbH_7y_wRT1M1CpNte0_8UiNHf6Is")
...
async def show_referral(update, context):
    ...
    # No Cancel/Close button. The main menu already has Referral and Support
    # side-by-side for quick navigation.
    ...
    if len(parts) >= 3:
        ...
        for line in str(content or "").splitlines():
            ...
            if re.match(r"^uid\s*:", line, re.I):
                display_lines.append(f"UID >> {escape(line.split(':',1)[1].strip())}")
            elif re.match(r"^(?:password|pass)\s*:", line, re.I):
                display_lines.append(f"PASS >> {escape(line.split(':',1)[1].strip())}")
            elif re.match(r"^2f\s*key\s*:", line, re.I):
                display_lines.append(f"2F KEY >> {escape(line.split(':',1)[1].strip())}")
            else:
                display_lines.append(escape(line))
    text = (
        "🛒 <b>Account Details</b>\n"
        "━━━━━━━━━━━━━━\n"
        + "\n\n".join(display_lines) + "\n"
...
async def next_purchase_callback(update, context):
    ...
    if secret:
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(
            "🔐 Get Code", callback_data=f"get_2fa:{stock['id']}")]])
